...
The missing beginner guide to [oh-my-pi](https://github.com/can1357/oh-my-pi) (`omp`): cheat sheets, tips and tricks, sane configs, and the traps that cost you an afternoon.
...
> Not an official publication. omp moves fast and defaults change. Everything here was verified on a live install of the release recorded in [`data/version.txt`](data/version.txt); a weekly job re-checks every new release and files an issue when anything documented here drifts. Run `omp config list` to see what *your* install actually does.
...
- Automated drift tracking: a weekly CI job installs the latest omp, snapshots its settings schema and CLI surface into [`data/`](data/), and opens an issue when anything documented here changes. See [How this stays current](#how-this-stays-current).
...
- A [weekly GitHub Action](.github/workflows/watch-omp.yml) installs the latest omp in a clean environment, regenerates the snapshots, commits the diff, and **opens an issue listing exactly which settings, defaults, commands, or flags changed**. Each issue is the review queue for updating the prose.
...
- The update itself is agent-driven: [`.omp/commands/update-docs.md`](.omp/commands/update-docs.md) is a versioned brief that compares the guide against a live install and rewrites every drifted page. In omp, run `/update-docs` from the repo root; in any other coding agent, paste the file as the prompt.
